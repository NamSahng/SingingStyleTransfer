...
def train(train_A_dir, train_B_dir, model_dir, model_name, random_seed, validation_A_dir, validation_B_dir, output_dir,
    ...
    #print("model = CycleGAN(num_features=24)")
    for epoch in range(num_epochs):
        ...
        dataset_A, dataset_B = sample_train_data(dataset_A=coded_sps_A_norm, dataset_B=coded_sps_B_norm,
                                                 n_frames=n_frames)
        print(np.shape(dataset_A[0]))
        
        n_samples = dataset_A.shape[0]
        # print("n_samples = dataset_A.shape[0]")
        ...
        for i in range(n_samples // mini_batch_size):
            ...
            generator_loss, discriminator_loss, measure_A, measure_B, k_t_A, k_t_B, balance_A, balance_B = model.train(
                input_A=dataset_A[start:end], input_B=dataset_B[start:end], lambda_cycle=lambda_cycle,
                lambda_identity=lambda_identity,
                gamma_A=gamma_A, gamma_B=gamma_B, lambda_k_A=lambda_k_A, lambda_k_B=lambda_k_B,
                generator_learning_rate=generator_learning_rate,
                discriminator_learning_rate=discriminator_learning_rate, k_t_A = k_t_A, k_t_B = k_t_B)
            ...
            if i % 50 == 0:
                ...
                'Iteration: {:07d}, Generator Learning Rate: {:.7f}, Discriminator Learning Rate: {:.7f}, Generator Loss : {:.3f}, Discriminator Loss : {:.3f}'.format(
                    num_iterations, generator_learning_rate, discriminator_learning_rate, generator_loss,
                    discriminator_loss))
                print(
                'Measure_A: {:.3f}, measure_B: {:.3f}, k_t_A: {:.3f}, k_t_B: {:.3f}'.format(measure_A, measure_B, k_t_A,
                                                                                            k_t_B))
            ...
            if epoch % 50 == 0:
                ...
                for file in os.listdir(validation_A_dir):
                    ...
                    try:
                        ...
                        librosa.output.write_wav(os.path.join(validation_A_output_dir, os.path.basename(file)),
            ...
            if epoch % 50 == 0:
                ...
                for file in os.listdir(validation_B_dir):
                    try:
                        ...
                        wav_transformed = world_speech_synthesis(f0=f0_converted, decoded_sp=decoded_sp_converted,
                                                                 ap=ap, fs=sampling_rate, frame_period=frame_period)
                        librosa.output.write_wav(os.path.join(validation_B_output_dir, os.path.basename(file)),
                                                 wav_transformed, sampling_rate)
